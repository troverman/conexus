...
var App = {
	...
	//TODO: REMOVE +REQUIRE --> DYNAMIC ::::NPM CODE:::: GOAL
	'CONNECTION+REQUEST': global['appApp']['GET+REQUIRE']('request'),
	//'CONNECTION+REQUEST-PROMISE':  global['appApp']['GET+REQUIRE']('request-promise'),
	'CONNECTION+ASYNC':  global['appApp']['GET+REQUIRE']('async'),
	...
	getGeoNamesByParent: function(geoNameParentId, parentId, username, nestLevel){
		...
		App['CONNECTION+REQUEST'](model, function (error, response, body) {
			if (body && body.geonames && body.geonames.length > 0){
				var nameData = body.geonames;
				App['CONNECTION+ASYNC'].eachSeries(nameData, function (projectData, nextProject){
	...
	getNamesWorld: function(){
		...
		App['CONNECTION+REQUEST'](model, async function (error, response, body) {
	...
	getGeoNames: function(model){
		...
		return App['CONNECTION+REQUEST'](requestModel, function (error, response, body) {
