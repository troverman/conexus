...
module.exports = function(sails) {
  ...
  return function load(configOverride, cb) {
    ...
    if (_.isFunction(configOverride)) {cb = configOverride; configOverride = {};}
    ...
    if (configOverride.host) {configOverride.explicitHost = configOverride.host;}
  ...
  function helpLoadHooks(results, cb) {
    ...
    if (sails.config.hooks === false) {return cb();}
    async.series([
  ...
  function loadHookDefinitions(hooks, cb) {
    ...
    if (sails.config.loadHooks) {
      ...
      _.each(hooks, function(def, hookName) {
        if (!_.contains(sails.config.loadHooks, hookName)) {hooks[hookName] = false;}
      });
  ...
  function ready__(cb) {
    return function(err) {
      ...
      if (sails.config.hooks && sails.config.hooks.userconfig === false || (sails.config.loadHooks && sails.config.loadHooks.indexOf('userconfig') === -1)) {sails.exposeGlobals();}
      // If the Sails environment is set to "production" but the Node environment isn't,
      // log a warning.
      if (sails.config.environment === 'production' && process.env.NODE_ENV !== 'production') {sails.log.warn('Detected Sails environment of `production`, but Node environment is `' + process.env.NODE_ENV + '`.\n' + 'It is recommended that in production mode, both the Sails and Node environments be set to `production`.');}
      cb && cb(null, sails);
    };
  }
};
