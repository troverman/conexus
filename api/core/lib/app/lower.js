...
module.exports = function lower(options, cb) {
  ...
  sails._exiting = true;
  var beforeShutdown = (sails.config && sails.config.beforeShutdown) || function(cb) {return cb();};
  ...
  beforeShutdown(function(err) {
    ...
    _.each(sails.childProcesses, function kill(childProcess) {
      ...
      try {childProcess.kill('SIGINT');} 
      catch (e) {sails.log.error('While lowering Sails app: received error killing child process:', e.stack);}
    });
    ...
    async.series([
      function shutdownSockets(cb) {
        ...
        if (!_.isObject(sails.hooks) || !sails.hooks.sockets || !sails.io || (sails.io && sails.io.httpServer && sails.hooks.http.server === sails.io.httpServer)) {return cb();}
        var timeOut;
        try {
      ...
      function shutdownHTTP(cb) {
        if (!_.isObject(sails.hooks) || !sails.hooks.http || !sails.hooks.http.server) {return cb();}
        var timeOut;
        try {
          ...
          if (sails.io && sails.io.httpServer && sails.hooks.http.server === sails.io.httpServer) {sails.io.close();}
          // If the "hard shutdown" option is on, destroy the server immediately,
          // severing all connections
          if (options.hardShutdown) {sails.hooks.http.destroy();}
      ...
      function removeListeners(cb) {
        ...
        _.each(_.keys(sails._events)||[], function (eventName){sails.removeAllListeners(eventName);});
