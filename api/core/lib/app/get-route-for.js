...
module.exports = function getRouteFor(routeQuery){
  ...
  var actionToLookup;
  try {actionToLookup = sails.router.getActionIdentityForTarget(routeQuery);} 
  ...
  var firstMatchingRouteAddress = _.find(_.keys(sails.router.explicitRoutes), function (address) {
    ...
    var actionIdentity;
    try {actionIdentity = sails.router.getActionIdentityForTarget(target);} 
    catch (e) {
      ...
      if (e.code === 'E_NOT_ACTION_TARGET') {return false;}
