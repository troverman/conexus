  ...
  includeAll.optional({
    ...
    try {
      ...
      _.each(files, function(moduleDef) {
        ...
        // path into slashes.
        if (path.dirname(filePath) !== '.') {filePath = path.dirname(filePath).replace(/\./g, '/') + '/' + path.basename(filePath);}
        ...
        if (match) {
          ...
          // throw it in the can.
          if (!_.isObject(moduleDef) || _.isArray(moduleDef) || _.isFunction(moduleDef)) {return garbage.push(filePath);}
          ...
          _.each(moduleDef, function(action, actionName) {
            ...
            if (actionName === '_config') {
              if (sails.config.blueprints) {sails.config.blueprints._controllers[identity.toLowerCase()] = action;}
              return;
            }
            ...
            if (actionsLoadedFromDisk[actionIdentity]) {throw flaverr({ name: 'userError', code: 'E_CONFLICT', identity: actionIdentity}, new Error('The action `' + actionName + '` in `' + filePath + '` conflicts with a previously-loaded action.'));}
            ...
            try {helpRegisterAction(sails, action, actionIdentity, true);} 
            catch (e) {
              switch (e.code) {
                case 'E_CONFLICT':
        ...
        // a function as its `fn` property.
          ...
          try {helpRegisterAction(sails, moduleDef, actionIdentity, true);}
          catch (e) {
            switch (e.code) {
        ...
        else {garbage.push(filePath);} // </ it is garbage>
