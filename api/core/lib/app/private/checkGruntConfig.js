...
module.exports = function(sails) {
  return function(results, cb) {
    ...
    if (sails.config.hooks.grunt === false) {return cb();}
    // Load this app's package.json and dependencies
    var appPackageJSON;
    try {appPackageJSON = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), 'package.json')));} 
    catch (unusedErr) {
