...
module.exports = function request( /* address, body, cb */ ) {
  ...
  if (arguments[2]) {cb = arguments[2]; body = arguments[1];}
  if (_.isFunction(arguments[1])) {cb = arguments[1];} 
  else if (arguments[1]) {body = arguments[1];}
  ...
  if (_.isObject(body) && _.contains(['GET', 'HEAD', 'DELETE'], method)) {
    ...
    if (queryStringPos === -1) {url += '?' + stringifiedParams;} 
    else {url = url.substring(0, queryStringPos) + '?' + stringifiedParams;}
  }
  ...
  clientRes.on('finish', function() {
    ...
    if (cb) {
      ...
      try {clientRes.body = clientRes.read(); clientRes.body = clientRes.body.toString();} 
      catch (unusedErr) {}
      ...
      if (!_.isUndefined(clientRes.body) && clientRes.headers['content-type'] === 'application/json') {clientRes.body = JSON.parse(clientRes.body);}
      ...
      else {return cb(undefined, clientRes, clientRes.body);}
    }
  });
...
MockClientResponse.prototype._transform = function(chunk, encoding, next) {this.push(chunk);next();};