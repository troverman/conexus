...
module.exports = function(app) {
  ...
  var SessionHook = {
    ...
    configure: function() {
      ...
      if (app.config.session.adapter === 'redis') {app.config.session.adapter = 'connect-redis';}
      else if (app.config.session.adapter === 'mongo') {app.config.session.adapter = 'connect-mongo';}
      ...
      if (_.isString(app.config.session.url)) {app.config.session.url = app.config.session.url.replace(/\/$/,'');}
    },
    ...
    initialize: function(cb) {
      ...
      try {
        ...
        app.hooks.session.routesDisabled = _.reduce(app.config.session.routesDisabled || [], function (memo, routeAddressStr){
          ...
          if (!_.isString(routeAddressStr)){throw new Error('Cannot parse route address (`'+routeAddressStr+'`). Must be a string.');}
          ...
          else {throw new Error('Cannot parse route address (`'+routeAddressStr+'`). When split on whitespace, there are either too many or too few pieces (`'+addrPieces.length+'`).');}
          ...
          if (matches) {urlPatternRegExp = new RegExp(matches[1]);} 
          else {urlPatternRegExp = pathToRegexp(urlPattern, []);}
          memo.push({
            method: method,
      ...
      (function setupAdapter(proceed) {
        ...
        // For all other adapters, we'll try to require the module and do some setup.
          ...
          (function maybeConnectToRedis(proceed) {
            ...
            Redis.createManager({
              ...
              onUnexpectedFailure: function(err) {
                ...
                if (err.failureType === 'end') {
                  if (_.isFunction(app.config.session.onRedisDisconnect)) {app.config.session.onRedisDisconnect();}
                  else {app.log.error('Redis session server went offline...');}
                  // If a disconnected client comes back, say something and run any custom logic
                  // that was provided for this occasion.
                  err.connection.once('ready', function() {
                    if (_.isFunction(app.config.session.onRedisReconnect)) {app.config.session.onRedisReconnect();} 
                    else {app.log.error('Redis session server came back online...');}
                  });
                }
              }
              ...
              Redis.getConnection({manager: createManagerResult.manager})
              .switch({
            ...
            if (_.isString(app.config.session.adapter)) {
              try {SessionAdapter = require(path.resolve(app.config.appPath, 'node_modules', app.config.session.adapter));}
              catch(rawRequireErr) {
            ...
            else if (_.isObject(app.config.session.adapter)) {SessionAdapter = app.config.session.adapter;}
            // Otherwise bail, because sails.config.session.adapter is invalid.
            else {return proceed(new Error('If configured, `sails.config.session.adapter` should be a reference to an Express session adapter!  Instead got `' + util.inspect(app.config.session.adapter)));}
        ...
        app._privateSessionMiddleware = function(req, res, next) {
          ...
          if(app.config.session.isSessionDisabled && app.config.session.isSessionDisabled(req)) {return next();}
    ...
    parseSessionIdFromCookie: function (cookie){
      ...
      if (typeof parsedSidCookie !== 'string') {throw flaverr({ status: 401, code: 'E_SESSION_PARSE_COOKIE' }, new Error('No sid cookie exists'));}//-•
      if (parsedSidCookie.substr(0, 2) !== 's:') {throw flaverr({ status: 401, code: 'E_SESSION_PARSE_COOKIE' }, new Error('Cookie unsigned'));}//-•
      // Unsign cookie
    ...
    get: function(sessionId, cb) {
      if (!_.isFunction(cb)) {throw new Error('Invalid usage :: `sails.hooks.session.get(sessionId, cb)`');}
      app.config.session.store.get(sessionId, function (err, session) {
        if (err) { return cb(err); }
        if (!session) {return cb(flaverr('E_SESSION', new Error('Session could not be loaded.')));}
    ...
    set: function(sessionId, data, cb) {
      if (!_.isFunction(cb)) {throw new Error('Invalid usage :: `sails.hooks.session.set(sessionId, data, cb)`');}
      ...
      app.config.session.store.set(sessionId, data, function (err) {
        ...
        app.config.session.store.get(sessionId, function (err, session) {
          if (err) { return cb(err); }
          if (!session) {return cb(flaverr('E_SESSION', new Error('Session (`'+sessionId+'`) could not be located after saving.')));}
          return cb(null, session);
        });//</store.get>
      });//</store.set>
    }
