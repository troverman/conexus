    ...
    (function _waitForSessionHookIfApplicable(next){
      ...
      try {
        ...
        _.each(['key', 'cert', 'pfx'], function _eachSSLOption(sslOption) {
          if (_.isArray(serverOptions[sslOption])) {serverOptions[sslOption] = new Buffer(serverOptions[sslOption]);}
        });
        ...
        var createServer = isUsingSSL ? require('https').createServer : require('http').createServer;
        // Use serverOptions if they were specified
        // Manually create http server using Express app instance
        if (sails.config.http.serverOptions || isUsingSSL) {sails.hooks.http.server = createServer(serverOptions, expressApp);}
        else {sails.hooks.http.server = createServer(expressApp);}
        ...
        sails.hooks.http.destroy = function(done) {
          ...
          for (var key in openTcpConnections) {openTcpConnections[key].destroy();}
        ...
        if (process.env.NODE_ENV !== 'production') {expressApp.set('json spaces', 2);}
        // Set Express "trust proxy" if appropriate.
        // > https://expressjs.com/en/guide/behind-proxies.html
        if (sails.config.http.trustProxy) {expressApp.set('trust proxy', sails.config.http.trustProxy);}
        ...
        expressApp.use(function _exposeSailsOnReq (req, res, next){
          ...
          req.param = function getValForParam (name){
            if (name === 'length') {
              ...
              if (_.isString(req.params.length)) {return req.params.length;}
              ...
              else if (_.isObject(req.query) && !_.isUndefined(req.query.length) && !_.isNull(req.query.length)) {return req.query.length;}
        ...
        if (_.contains(removedHttpMiddleware, 'startRequestTimer') || sails.config.http.middleware.startRequestTimer) {
          ...
          if (sails.config.http.middleware.startRequestTimer) {sails.log.debug('(ignoring custom implementation in `sails.config.http.middleware.startRequestTimer)');} 
          else {sails.log.debug('(ignoring entry in the `sails.config.http.middleware.order` list)');}
        ...
        if (_.contains(removedHttpMiddleware, '404') || sails.config.http.middleware['404']) {
          ...
          if (sails.config.http.middleware['404']) {sails.log.debug('(ignoring custom implementation in `sails.config.http.middleware[\'404\']`)');} 
          else {sails.log.debug('(ignoring entry in the `sails.config.http.middleware.order` list)');}
        ...
        if (_.contains(removedHttpMiddleware, '500') || sails.config.http.middleware['500']) {
          ...
          if (sails.config.http.middleware['500']) {sails.log.debug('(ignoring custom implementation in `sails.config.http.middleware[\'500\']`)');} 
          else {sails.log.debug('(ignoring entry in the `sails.config.http.middleware.order` list)');}
        ...
        _.each(sails.config.http.middleware.order, function(middlewareKey) {
          ...
          else if ( _.isArray(postRouterMiddleware) ) {postRouterMiddleware.push(middlewareKey);}
          else {preRouterMiddleware.push(middlewareKey);}
        });
        ...
        if (sails.config.http.loadMiddleware) {sails.config.http.loadMiddleware(expressApp, configuredHttpMiddlewareFns, sails);}
