...
function bindAction(path, target, verb, options) {
  ...
  var sails = this.sails;
  var actionIdentity;
  try {actionIdentity = self.getActionIdentityForTarget(target);} 
  catch (e) {throw flaverr({name: e.name || 'sailsError', code: e.code || 'E_UNKNOWN_BIND_ERROR'}, new Error('Error attempting to bind `' + (verb || 'ALL') + ' ' + path + '` to ' + JSON.stringify(target) + ': ' + e.message));}
  ...
  bind.apply(this,[path, function(req, res, next) {
    ...
    // bail out early.
    if (!_.isFunction(sails._actions[actionIdentity])) {return next(new Error('Consistency violation: Request (' + req.method + ' ' + req.path + ') matched a route that is bound to action `' + actionIdentity + '`, but no such action has been registered.  This never should have happened, because the route never should have been bound in the first place.'));}
...
function bindArray(path, target, verb, options) {
  var self = this;
  var sails = this.sails;
  if (target.length === 0) {sails.log.verbose('Ignoring empty array in `router.bind(' + path + ')`...');} 
  else {
...
function bindFunction(path, fn, verb, options) {
  ...
  else {targetFn = enhancedFn;}
  // If options.skipRegex is specified, make sure it's an array
  if (options.skipRegex) {
    if (!Array.isArray(options.skipRegex)) {options.skipRegex = [options.skipRegex];}
  }
...
function sanitize(path, target, verb, options) {
  ...
  if (!verb) {verb = detectedVerb.verb;}
  return {
    path: path,
    target: target,
    verb: verb,
