...
//TODO: APP CENTRIC ROUTING .. --> RESTRUCT
...
Router.prototype.load = function(results, cb) {
  ...
  //ROUTES ARE COMPILED FROM THE SET OF APPS .ROUTES
  ...
  if (!sails.hooks.session) {
    ...
    if (sessionSecret) {
      ...
      if (!_.isString(sessionSecret)) {return cb(new Error('If provided, sails.config.session.secret should be a string.'));}
    }
  }
  if (sessionSecret) {sails._privateCpMware = require('cookie-parser')(sessionSecret);} 
  else {sails._privateCpMware = require('cookie-parser')();}
...
Router.prototype.route = function(req, res) {
  ...
  // method complains.
  if (req.method === 'OPTIONS' && !res.statusCode) {res.status(200);}
  ...
  qsParser(req,res, function (err) {
    if (err) {return res.status(400).send(err && err.stack);}
    // Parse cookies
...
Router.prototype.flush = function(routes) {
  ...
  if (routes) {this.explicitRoutes = routes;}
...
Router.prototype.getActionIdentityForTarget = function getActionIdentityForTarget(target) {
  ...
  if (_.isObject(target) && !_.isArray(target) && !_.isFunction(target)) {
    ...
    if (target.controller) {
      if (!target.action) {throw flaverr({name: 'userError', code: 'E_NOT_ACTION_TARGET'}, new Error('If `controller` is specified, `action` must be also!'));}
    ...
    else {throw flaverr({name: 'userError', code: 'E_NOT_ACTION_TARGET'}, new Error('If target is a dictionary, it must contain an `action` property!'));}
    ...
    if (!actionIdentity.match(/^[a-zA-Z_\$]+[a-zA-Z0-9_\/\-\$]*$/)) {
      ...
      if (!actionIdentity.match(RX_DOESNT_HAVE_ANY_WEIRD_CHARS)) {didYouMean = ' Did you mean `' + actionIdentity.replace('Controller', '').replace(/\./g,'/') + '`?';}
  ...
  // 'user/create'
    ...
    actionIdentity = (function(){
      var actionTarget = _.find(target, function(targetComponent) {return targetComponent.action;});
      if (actionTarget) {return getActionIdentityForTarget(actionTarget);}
  ...
  else {throw flaverr({name: 'userError', code: 'E_NOT_ACTION_TARGET'}, new Error('Target must be a dictionary or string!'));}
...
function qsParser(req,res,next) {
  ...
  if (queryStringPos !== -1) {req.query = _.merge(req.query, QS.parse(req.url.substr(queryStringPos + 1)));}
  else {req.query = req.query || {};}
  next();
}
...
function bodyParser (req, res, next) {
  ...
  if ( !(req instanceof Readable) ) {return next(new Error('Sails Internal Error: `req` should be a Readable stream by the time `route()` is called'));}
  req.on('readable', function() {
    var chunk;
    while (null !== (chunk = req.read())) {bodyBuffer += chunk;}
  });
  req.on('end', function() {
    var parsedBody;
    try {parsedBody = JSON.parse(bodyBuffer);} 
    catch (unusedErr) {}
