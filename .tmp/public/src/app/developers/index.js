...
.config(['$stateProvider', 'hljsServiceProvider', function config( $stateProvider, hljsServiceProvider) {
    $stateProvider.state( 'developers', {
        url: '/developers',
        views: {"main": {controller: 'DevelopersController', templateUrl: 'developers/index.tpl.html'}}
    });
    //TODO
    ...
    $scope.newOrderModel = JSON.stringify([
        {
            ...
            setBeta:{'CRE8+TIME+REST+ONMINT+SPONSOR+id':0.1}, 
            type:'ONBOOK', 
            status:'CONTINUAL'
        },{
            setAlpha:{UNIVERSALTOKEN:15},
            setBeta:{
                'CRE8+TIME+CHILL+ONMINT+SPONSOR+id':2, 
                'CRE8+TIME+VOLUNTEER+ONMINT+SPONSOR+id':1
            }, 
            type:'ONBOOK', 
            status:'CONTINUAL'
        },
