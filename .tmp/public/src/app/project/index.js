...
.config(['$stateProvider', function config( $stateProvider ) {
	$stateProvider.state( 'project', {
        abstract: true,
        url: '/project/:path',
        views: {"main": {controller: 'ProjectCtrl', templateUrl: 'project/index.tpl.html'}},
        resolve: {project: ['$stateParams', 'ProjectModel', function($stateParams, ProjectModel) {return ProjectModel.get({urlTitle:$stateParams.path,limit:1,skip:0,sort:'createdAt DESC'});}]}
    })
    .state( 'project.about', {
        url: '/about',
        views: {"projectAbout": {controller: 'ProjectAboutCtrl', templateUrl: 'project/templates/about.tpl.html'}}
    })
    .state( 'project.activity', {
        url: '',
        views: {"projectActivity": {controller: 'ProjectActivityCtrl', templateUrl: 'project/templates/activity.tpl.html'}}
    })
    .state( 'project.assets', {
        url: '/assets',
        views: {"projectAssets": {controller: 'ProjectAssetsCtrl', templateUrl: 'project/templates/assets.tpl.html'}}
    })
    .state( 'project.channels', {
        url: '/channels',
        views: {"projectChannels": {controller: 'ProjectChannelsCtrl', templateUrl: 'project/templates/channels.tpl.html'}}
    })
    .state( 'project.content', {
        url: '/content',
        views: {"projectContent": {controller: 'ProjectContentCtrl', templateUrl: 'project/templates/content.tpl.html'}},
        resolve: {
            //TODO-CONNECTION--BETTER PARAMS
            contentList: ['AssociationModel', 'project', function(AssociationModel, project){
                var query = {filter:JSON.stringify({type:'CONTENT', id:project.id}), limit:100, skip:0, sort:'createdAt DESC'};
    ...
    .state( 'project.charter', {
        url: '/charter',
        views: {"projectCharter": {controller: 'ProjectCharterCtrl', templateUrl: 'project/templates/charter.tpl.html'}},
        resolve: {
            connections: ['AssociationModel', 'project', function(AssociationModel, project){
                var query = {filter:JSON.stringify({type:'CONNECTION', id:project.id}), limit:100, skip:0, sort:'createdAt DESC'};
                return AssociationModel.get(query);
            }]
        }
    })
    .state( 'project.ledger', {
        url: '/ledger',
        views: {"projectLedger": {controller: 'ProjectLedgerCtrl', templateUrl: 'project/templates/ledger.tpl.html'}},
        //TODO: BETTER QUERY
        resolve: {
            transactions: ['TransactionModel', 'project', function(TransactionModel, project) {return TransactionModel.get({project:project.id, limit:100, skip:0, sort:'createdAt DESC'});}],
            transactionsFrom: ['TransactionModel', 'project', function(TransactionModel, project) {return TransactionModel.get({from:project.id, limit:100, skip:0, sort:'createdAt DESC'});}],
            transactionsTo: ['TransactionModel', 'project', function(TransactionModel, project) {return TransactionModel.get({to:project.id, limit:100, skip:0, sort:'createdAt DESC'});}],
        }
    })
    .state( 'project.items', {
        url: '/items',
        views: {"projectItems": {controller: 'ProjectItemsCtrl', templateUrl: 'project/templates/items.tpl.html'}},
        resolve: {
            items: ['AssociationModel', 'project', function(AssociationModel, project) {
                var query = {filter:JSON.stringify({type:'ITEM', id:project.id}), limit:100, skip:0, sort:'createdAt DESC'};
                return AssociationModel.get(query);
            }]
        }
    })
    .state( 'project.members', {
        url: '/members',
        views: {"projectMembers": {controller: 'ProjectMembersCtrl', templateUrl: 'project/templates/members.tpl.html'}},
        resolve: {
            members: ['AssociationModel', 'project', function(AssociationModel, project) {
                var query = {filter:JSON.stringify({type:'MEMBER', id:project.id}), limit:100, skip:0, sort:'createdAt DESC'};
                return AssociationModel.get(query);
            }]
        }
    })
    .state( 'project.positions', {
        url: '/positions',
        views: {"projectPositions": {controller: 'ProjectPositionsCtrl', templateUrl: 'project/templates/positions.tpl.html'}},
        resolve: {
            orders: ['OrderModel', 'project', function(OrderModel, project){
                var query = {filter:JSON.stringify({type:'ORDER', id:project.id}), limit:100, skip:0, sort:'createdAt DESC'};
                return AssociationModel.get(query);
            }],
        }
    })
    .state( 'project.projects', {
        url: '/projects',
        views: {"projectProjects": {controller: 'ProjectProjectsCtrl', templateUrl: 'project/templates/projects.tpl.html'}},
        resolve: {
            projects: ['AssociationModel', 'project', function(AssociationModel, project){
                var query = {filter:JSON.stringify({type:'PROJECT', id:project.id}), limit:100, skip:0, sort:'createdAt DESC'};
                return AssociationModel.get(query);
            }],
        }
    })
    .state( 'project.settings', {
        url: '/settings',
        views: {"projectSettings": {controller: 'ProjectSettingsCtrl', templateUrl: 'project/templates/settings.tpl.html'}}
    })
    .state( 'project.tasks', {
        url: '/tasks',
        views: {"projectTasks": {controller: 'ProjectTasksCtrl', templateUrl: 'project/templates/tasks.tpl.html'}},
        resolve: {
            tasks: ['AssociationModel', 'project', function(AssociationModel, project){
                var query = {filter:JSON.stringify({type:'TASK', id:project.id}), limit:100, skip:0, sort:'createdAt DESC'};
                return AssociationModel.get(query);
            }],
        }
    })
    .state( 'project.time', {
        url: '/time',
        views: {"projectTime": {controller: 'ProjectTimeCtrl', templateUrl: 'project/templates/time.tpl.html'}},
        resolve: {
            time: ['AssociationModel', 'project', function(AssociationModel, project) {
                var query = {filter:JSON.stringify({type:'TIME', id:project.id}), limit:100, skip:0, sort:'createdAt DESC'};
                return AssociationModel.get(query);
            }]
        }
    })
    .state( 'project.validations', {
        url: '/validations',
        views: {"projectValidations": {controller: 'ProjectValidationsCtrl', templateUrl: 'project/templates/validations.tpl.html'}}
    })
}])
.controller( 'ProjectCtrl', ['$location', '$mdSidenav', '$rootScope', '$scope', '$state', 'project', 'SearchModel', 'titleService', 'TransactionModel', 'ValidationModel', function ProjectController( $location, $mdSidenav, $rootScope, $scope, $state, project, SearchModel, titleService, TransactionModel, ValidationModel ) {
    ...
    if ($scope.project.location){ 
        $scope.map = {center: {latitude: $scope.project.location.lat, longitude: $scope.project.location.lng }, zoom: 9};
        $scope.markers = [{
            id:project.id,
            content:$scope.project.title,
            coords:{latitude:$scope.project.location.lat, longitude:$scope.project.location.lng}
        }];
    ...
    $scope.createMemberValidate = function(){
        if($scope.currentUser){
            $scope.newValidation = {
                ...
                //TODO: CONNECTION
                associatedModels:[{type:'MEMBER', id:$rootScope.currentUser.id}, {type:'PROJECT', id:project.id}],
                context:{general:100},
                connection:{},
                information:{},
                parameters:{context:{general:100}},
            };
            ValidationModel.create($scope.newValidation).then(function(model) {console.log(model);});
...
.controller( 'ProjectActivityCtrl', ['$location', '$rootScope', '$sailsSocket', '$scope', 'EventModel', 'project', 'SearchModel', 'titleService', function ProjectActivityController( $location, $rootScope, $sailsSocket, $scope, EventModel, project, SearchModel, titleService) {
    ...
    $scope.project = project;
    //TODO: EVENTS
    //var query = {limit:100, skip:0, sort:'createdAt DESC', filter:[{}], model:'ASSOCIATION', id:project.id, };
...
.controller( 'ProjectAssetsCtrl', ['$scope', 'project', 'titleService', function ProjectAssetsController( $scope, project, titleService ) {
    ...
    $scope.balance = {};
    if ( $scope.project.data){$scope.balance = $scope.project.data.apps.balance;}
    $scope.balanceColumn = {
        chart: {zoomType: 'x',},
...
.controller( 'ProjectLedgerCtrl', ['$interval', '$location', '$rootScope', '$scope', 'project', 'titleService', 'TransactionModel', 'transactions', 'transactionsFrom', 'transactionsTo', function ProjectController( $interval, $location, $rootScope, $scope, project, titleService, TransactionModel, transactions, transactionsFrom, transactionsTo ) { 
    ...
    $scope.pie = {
        chart: {},
        series: [{id: 'Pie', type: 'pie', name: 'Pie', colorByPoint: true, data: []}],
        title: {text: ''},
    ...
    if ($scope.transactions.length == 0 || !$scope.transactions){
        ...
        for (var i=0, t=88; i<t; i++) {
            ...
            $scope.transactionsFrom.push({
                ...
                amountSet:{CRE8: 10*Math.round(0.5*Math.random() * t)}, 
    ...
    function amountInArray(array, value) {return array.reduce(function(n, x){ if (x.tag == value.tag){n+=parseFloat(x.amount)} return n;}, 0);};
    //TODO: IMPROVE :)
    ...
    $scope.updateGraph = function(){
        ...
        function sumFunction(obj){
            var sumArray = [];
            obj.reduce(function(a,b,i) {return $scope.sumArray[i] = parseFloat(a) + parseFloat(b.amount);}, 0);
            return sumArray;
        };
    ...
    $scope.selectOverview = function(){
        $scope.pie.series[0].data = [];
        $scope.pie.series[0].data = [{name: 'Expense', y: $scope.sumFrom[$scope.sumFrom.length-1][1]}, {name: 'Revenue', y: $scope.sumTo[$scope.sumTo.length-1][1]}];
    };
    ...
    $scope.selectTag = function(tag){
        ...
        TransactionModel.get('query', query, 20, 0, 'createdAt DESC').then(function(transactions){$scope.transactions = transactions;});
    };
    //TODO: TAGS
...
.controller( 'ProjectPositionsCtrl', ['$location', '$rootScope', '$sailsSocket', '$scope', '$stateParams', 'OrderModel', 'orders', 'project', 'titleService', function MemberPositionsController($location, $rootScope, $sailsSocket, $scope, $stateParams, OrderModel, orders, project, titleService) {
    ...
    $scope.project = project;
    $scope.orders = orders;
    $rootScope.baseMarkets = [{text:'UNIVERSAL'}, {text:'CRE8'},];
    //TODO: MULTIORDER MANIFOLDS
    $rootScope.markets = [{text:'EDUCATION+ONMINT'}, {text:'SHELTER'}, {text:'HEALTH'}, {text:'FOOD'}, {text:'REST'}, {text:'CRE8'}, {text:'USD'}, {text:'ETH'}, {text:'BTC'}, {text:'CREATE'}, {text:'TIME'}, {text:'STREAM'}, {text:'REACT'}, {text:'CONTENT'}, {text:'ORDER'}, {text:'TRANSACTION'}, {text:'VIEW'}];
        ...
        if (locationProjects.length > 0){
            ...
            for (x in locationProjects){
                $rootScope.markers.push({
                    ...
                    coords:{latitude:locationProjects[x].location.lat, longitude:locationProjects[x].location.lng}
                });
            }
        }
    }
