...
.config(['$stateProvider', function config( $stateProvider ) {
    $stateProvider.state( 'order', {
        url: '/order/:id',
        views: {"main": {controller: 'OrderController', templateUrl: 'order/index.tpl.html'}},
        //TODO: DEPRECIATE RESOLVE
        resolve: {
            order: ['$stateParams', 'OrderModel', function($stateParams, OrderModel){return OrderModel.get({id:$stateParams.id, limit:1, skip:0, sort:'createdAt DESC'});}],
            orders:['order', 'OrderModel', function(order, OrderModel){return OrderModel.get({order:order.id, limit:100, skip:0, sort:'createdAt DESC'});}],
            contentList: ['ContentModel', 'order', function(ContentModel, order){return ContentModel.get({order:order.id, limit:100, skip:0, sort:'createdAt DESC'});}],
        }
    });
}])
