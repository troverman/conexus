...
.config(['$stateProvider', 'uiGmapGoogleMapApiProvider', function config( $stateProvider, uiGmapGoogleMapApiProvider ) {
	$stateProvider.state( 'transparency', {
		url: '/transparency',
		views: {"main": {controller: 'TransparencyCtrl', templateUrl: 'transparency/index.tpl.html'}},
        resolve: {peers: ['PeerModel', function(PeerModel){return PeerModel.get({limit:100, skip:0, sort:'createdAt DESC'});}],}
	});
}])
.controller( 'TransparencyCtrl', ['$scope', 'titleService', 'peers', function TransparencyController( $scope, titleService, peers ) {
    ...
    projects.reduce(function(a,b,i) {return projects[i] = [new Date(b[0]).getTime(), parseFloat(a[1])+parseFloat(b[1])];},[0,0]);
    members.reduce(function(a,b,i) {return members[i] = [new Date(b[0]).getTime(), parseFloat(a[1])+parseFloat(b[1])];},[0,0]);
    orders.reduce(function(a,b,i) {return orders[i] = [new Date(b[0]).getTime(), parseFloat(a[1])+parseFloat(b[1])];},[0,0]);
    transactions.reduce(function(a,b,i) {return transactions[i] = [new Date(b[0]).getTime(), parseFloat(a[1])+parseFloat(b[1])];},[0,0]);
    tokens.reduce(function(a,b,i) {return tokens[i] = [new Date(b[0]).getTime(), parseFloat(a[1])+parseFloat(b[1])];},[0,0]);
    peers.reduce(function(a,b,i) {return peers[i] = [new Date(b[0]).getTime(), parseFloat(a[1])+parseFloat(b[1])];},[0,0]);
    ...
    $scope.map = {center: {latitude: 35.902023, longitude: -84.1507067}, zoom: 2};
