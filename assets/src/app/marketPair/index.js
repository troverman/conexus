...
.controller( 'MarketPairCtrl', ['$scope', '$stateParams', 'config', 'OrderModel', 'orders', 'titleService', function MarketPairController( $scope, $stateParams, config, OrderModel, orders, titleService ) {
    ...
    for(var i=-1000;i<1000;i++){
        if (i>0){$scope.bidAskChart.series[1].data.push([i+100000,i*i*i]);$scope.bidAskChart.series[0].data.push([i+100000,0])}
        if (i<0){$scope.bidAskChart.series[0].data.push([i+100000,i*i*i]);$scope.bidAskChart.series[1].data.push([i+100000,0])}
